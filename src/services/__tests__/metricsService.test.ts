...
describe('MetricsService', () => {
  ...
  test('should record retry attempts', async () => {
    // Start a session
    await metricsService.startSession(1);
    
    // Record a retry practice
    const practiceData = {
      wordPair: {
        id: 'pair1',
        word1: 'bat',
    ...
    expect(updatedSession.successfulRetries).toBe(1);
  });
  ...
  test('should get contrast accuracy', async () => {
    ...
    expect(dbService.queryPracticeResults).toHaveBeenCalledWith({ contrastType: ContrastType.PLOSIVE_VOICED_UNVOICED });
  });
});