...
const ProgressChart: React.FC<ProgressChartProps> = ({ data, chartTitle }) => {
  ...
  const chartData = data.map(stat => {
    ...
    const label = stat.contrastType.split('-').map(word => 
    ...
    return {
      ...
      retrySuccess: Math.round(stat.retrySuccessRate),
      attempts: stat.totalPractices
    };
  });
  ...
  return (
    <div className="w-full">
      ...
      <div className="text-sm text-gray-500 mt-2">
        Based on {chartData.reduce((sum, item) => sum + item.attempts, 0)} total attempts
      </div>
    </div>
  );
};
